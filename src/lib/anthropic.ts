...
/* ── Document AI OCR structuring prompt ──────────────────────────────── */

export const DOCAI_STRUCTURING_PROMPT = `You are given OCR text extracted from an architectural floor plan by Google Document AI. The text preserves spatial layout — room labels, dimension annotations, and notes appear roughly in their physical positions on the page.

Using ONLY the OCR text below, produce a structured room analysis JSON for HVAC load calculations.

Rules:
- Use the exact room labels from the OCR text as the "name" field
- Map each label to the closest room type enum: master_bedroom | bedroom | living_room | family_room | kitchen | dining_room | bathroom | half_bath | hallway | laundry | office | foyer | sunroom | bonus_room | basement | closet | garage
- Read dimension annotations to determine width_ft and length_ft for each room
- Convert feet-inches to decimal feet (e.g., 12'-6" = 12.5)
- Compute estimated_sqft as width_ft × length_ft
- Verify room sqft sum is within 10% of the total building sqft from the OCR text
- For rooms with no dimensions in the OCR text, estimate from neighboring rooms and the total sqft budget. Note "dimensions estimated from OCR context" in the notes field.
- Assign each room to the correct floor number (1, 2, 3, etc.)
- For multi-unit buildings: identify each unit, tag every room with its unit number (1-indexed), and populate the "unit_sqft" array with conditioned sqft per unit
- If units appear identical, analyze one unit only and note that
- Exclude garages, patios, porches, and unconditioned spaces from the rooms array
- Do NOT invent dimensions that are not present in the OCR text
...
/* ── Hybrid prompt: OCR text + floor plan images ─────────────────────── */

export const DOCAI_HYBRID_PROMPT = `You are analyzing architectural floor plan images with the help of OCR text extracted by Google Document AI.

You have TWO sources of information:
1. **The floor plan images above** — use these to see the spatial layout, identify rooms, count windows, spot exterior walls, and read any rotated/angled dimension annotations that OCR may have missed.
2. **The OCR text below** — use this for accurate readings of horizontal text: room labels, dimension values, square footage callouts, scale notations, and notes. OCR is more accurate than reading text from images, but it misses rotated annotations.

**Strategy:** Start with the OCR text for all readable dimensions and labels. Then examine the images to fill in anything the OCR missed — especially dimensions that run vertically along walls, which OCR often skips.

Rules:
- Use the exact room labels as the "name" field
- Map each label to the closest room type enum: master_bedroom | bedroom | living_room | family_room | kitchen | dining_room | bathroom | half_bath | hallway | laundry | office | foyer | sunroom | bonus_room | basement | closet | garage
- Read dimension annotations from OCR text first, then check images for any the OCR missed (especially rotated text)
- Convert feet-inches to decimal feet (e.g., 12'-6" = 12.5)
- Compute estimated_sqft as width_ft × length_ft
- Verify room sqft sum is within 10% of the total building sqft
- For rooms with missing dimensions, derive from neighboring rooms using chain dimensions and the total sqft budget. Note "dimensions derived" in the notes field.
- Assign each room to the correct floor number (1, 2, 3, etc.)
- For multi-unit buildings: identify each unit, tag every room with its unit number (1-indexed), and populate the "unit_sqft" array with conditioned sqft per unit
- If units appear identical, analyze one unit only and note that
- Exclude garages, patios, porches, and unconditioned spaces from the rooms array
- Count window symbols from the images (parallel lines in walls with gaps)
- Count exterior walls from the images (thicker outer walls)
